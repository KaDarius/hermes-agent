"""Contract checks for the bundled codex-build-agent skill asset.
...
def test_frontmatter_required_fields():
    ...
    assert fm["name"] == "codex-build-agent"
...
def test_related_skills_resolve_in_repo():
    fm, _ = _frontmatter_and_body()
    related = fm.get("related_skills", "").strip("[]")
    for skill in related.split(","):
        skill = skill.strip()
        if not skill:
            continue
        assert (REPO_ROOT / "skills" / "autonomous-ai-agents" / skill / "SKILL.md").is_file(), (
            f"related_skill {skill} does not resolve in-repo"
        )
